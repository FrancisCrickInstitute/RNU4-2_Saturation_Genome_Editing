#210317_sam_to_edits_pipeline.py
...
def sam_to_variant_cigar_dict(sam_file, min_alignment_score):
	...
	for line in sam_file:
		if line_count < 0:
			...
			#print(sam_data[5])
			cigar = sam_data[5]
			#this is the alignment score -- not necessarily useful unless mispriming common (this was 11 before)
			#AS = sam_data[12]
			#print(line)
			AS = sam_data[11]
			#sequence of read
...
#rewrite this header string to incorporate sample names from amplicon list?
...
for exp in exp_groupings.keys():
	...
	print(str(len(exp_variants))+' total exp. variants (iclu. lib)')
